...
console.log("🔹 Step 0: get intro show flag");introFlags=await loadStep([checkIntroFlags(),loadCSS("https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css"),loadScript("https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js")],signal);console.log("skipping intro");removeIntro();break intro;console.log("🔹 Step 1: processing returned or skipped");if(introFlags.includes("returned")){await loadStep([prepareSkipButtons(".slide-container.zero-slide ~ [class*='-slide']",controller),],signal);}
else if(introFlags.includes("skip")){console.log("skipping intro");removeIntro();break intro;}
console.log("🔹 Step 2: load zero slide css");await loadStep([loadCSS("css/zero.min.css"),],signal);document.body.style.display='block';console.log("🔹 Step 3: load lazy load js");await loadStep([loadScript("https://cdn.jsdelivr.net/npm/vanilla-lazyload@19.1.3/dist/lazyload.min.js","LazyLoad"),],signal);console.log("🔹 Step 4: load zero slide resources");await loadStep([loadResources(".slide-container.zero-slide .lazy",null),],signal);loadedProgressSteps++;updateProgress(0,allProgressSteps);console.log("🔹 Step 5: load main css");await loadStep([loadCSS("css/style.min.css"),loadScript("https://cdnjs.cloudflare.com/ajax/libs/fitty/2.3.2/fitty.min.js","fitty")],signal);loadedProgressSteps++;updateProgress(0,allProgressSteps);console.log("🔹 Step 6: load intro slides resources");const step6Results=await loadStep([loadResources(".slide-container.zero-slide ~ [class*='-slide'] .lazy",updateProgress),loadCSS("https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css"),loadScript("https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"),initSprite()],signal);console.log("🔹 Step 7: waiting resources progress");await loadStep([waitForProgressCompete(),],signal);loadCSS("css/element-icons.css");console.log("🔹 Step 8: blue part animation");await loadStep([showBluePartAnimation(),],signal);console.log("🔹 Step 9: hide zero slide and begin slideshow");await loadStep([removeSlide(document.querySelector(".slide-container.zero-slide")),startSlidesCycle(".slide-container.zero-slide ~ [class*='-slide']",step6Results[3])],signal);}
catch(error){if(error.name==="AbortError"){console.log("⏩ Loading skipped by user");}else{console.error("❌ Error loading page:",error);}}
controller=new AbortController();signal=controller.signal;try{console.log("🔹 Step 10: loading main page scripts");await loadStep([loadScript("https://cdn.jsdelivr.net/npm/vanilla-lazyload@19.1.3/dist/lazyload.min.js","LazyLoad"),loadCSS("https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css"),loadScript("https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js")],signal);console.log("🔹 Step 11: loading main page resources");await loadStep([loadCSS("css/style.min.css"),loadResources(".video-banner-poster-lazy")],signal);console.log("🔹 Step 12: showing page and lazyload remaining resources");await loadStep([initLazyLoad(),enableMainPage(),showMainPage(),loadCSS("css/element-icons.css")],signal);}
catch(error){if(error.name==="AbortError"){console.log("⏩ Loading skipped by user");await showMainPage();}else{console.error("❌ Error loading page:",error);}}}
...
const onAbort=()=>{reject(new DOMException("Aborted","AbortError"));};signal?.addEventListener("abort",onAbort,{once:true});const validPromises=promises.filter(p=>p instanceof Promise);const results=Promise.all(validPromises).then(resolve).catch(reject).finally(()=>{signal?.removeEventListener("abort",onAbort);});return results.length===1?results[0]:results;});}
...
function removeIntro(){const introSlides=document.querySelectorAll("body > [class*='-slide']");if(introSlides){introSlides.forEach(slide=>{removeSlide(slide);});}}
function skipIntro(){const slides=document.querySelectorAll("body > [class*='-slide']");slides.forEach((slide)=>{removeSlide(slide);});}
function fixSlideshowVideoSources(selector){return new Promise((resolve,reject)=>{const slideshowVideo=document.querySelector(selector);if(!slideshowVideo){reject();return;}
const sources=Array.from(slideshowVideo.querySelectorAll("source"));const supportsMP4=document.createElement("video").canPlayType("video/mp4")!=="";let bestSourceMP4=null;let bestSourceWebM=null;sources.forEach(source=>{const type=source.getAttribute("type");const media=source.getAttribute("media")||"";let matchesMedia=!media||window.matchMedia(media).matches;if(matchesMedia){if(type==="video/mp4"){bestSourceMP4=source;}else if(type==="video/webm"){bestSourceWebM=source;}}});const bestSource=supportsMP4?bestSourceMP4||bestSourceWebM:bestSourceWebM;if(bestSource){console.log(`Выбрано видео: ${bestSource.getAttribute("data-src")}`);slideshowVideo.setAttribute("data-src",bestSource.getAttribute("data-src"));slideshowVideo.querySelectorAll("source").forEach(source=>source.remove());resolve();}
reject();});}
...
async function initLazyLoad(){return new Promise((resolve,reject)=>{const lazyLoad=new LazyLoad({elements_selector:".lazy-main",use_native:"true"});resolve();});}
...
async function showMainPage(){return new Promise((resolve,reject)=>{const mainPage=document.querySelector(".main-page");const navbar=document.getElementById("mainNavbar");const roundButton=document.getElementById('round-center-button');document.body.style.backgroundColor="var(--text-light-color-hover)";if(mainPage){mainPage.style.animation='none';mainPage.style.display='block';mainPage.offsetHeight;mainPage.style.animation='showMainPage 100ms linear forwards, reduceHeightMainPage var(--main-page-delay) ease-out forwards';}
if(navbar){navbar.style.animation='none';navbar.offsetHeight;navbar.style.animation='moveDownNavBar var(--main-page-delay) ease-out 1 normal forwards';}
if(roundButton){roundButton.style.animation='none';roundButton.offsetHeight;roundButton.style.animation='fadeInButton var(--main-page-delay) ease-out 1 normal forwards';}
const mainpageTextRows=mainPage.querySelectorAll(".text-row");mainpageTextRows.forEach(row=>{row.style.display='none';row.style.animation='none !important';row.style.transition='none !important';});resolve();});}
document.addEventListener('DOMContentLoaded',()=>{if(!startTime){startTime=performance.now();}
loadInto();const navbar=document.getElementById("mainNavbar");const videoBanner=document.querySelector(".video-banner");const toggler=document.querySelector('.navbar-toggler');const navbarmenu=document.querySelector('.navbar-collapse');const navbarCollapse=document.getElementById('navbarToggler');const video=document.getElementById('vid');const roundButton=document.getElementById('round-center-button');window.addEventListener('scroll',updateScrolledGradient);window.addEventListener('scroll',closeMenu);if(navbarCollapse){navbarCollapse.addEventListener('show.bs.collapse',hideRoundButton);navbarCollapse.addEventListener('hide.bs.collapse',showRoundButton);}
...
if(video){video.addEventListener('canplaythrough',()=>{const poster=document.querySelector(".video-banner-poster");const videoBannerVideo=document.querySelector(".video-banner video");videoBannerVideo.style.display="block";poster.style.display="none";video.play();},{once:true});}
...
function autoResize(event){const textarea=event?event.target:messageInput;textarea.style.height='auto';textarea.style.height=textarea.scrollHeight+'px';}
