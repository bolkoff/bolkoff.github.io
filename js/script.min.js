...
async function checkIntroFlags(){function getLocalStorageValue(key){let value=localStorage.getItem(key)?parseInt(localStorage.getItem(key),10)||0:0;return value;}
...
const mainpageTextRows=mainPage.querySelectorAll(".text-row");mainpageTextRows.forEach(row=>{row.style.display='none';row.style.animation='none !important';row.style.transition='none !important';});}
resolve();});}
function adjustVimeoSize(){const container=document.querySelector(".video-banner");const poster=document.querySelector(".video-banner .video-banner-poster");const iframe=container.querySelector("iframe");const player=new Vimeo.Player(iframe);function resizeVideo(){player.getVideoWidth().then(videoWidth=>{return player.getVideoHeight().then(videoHeight=>{const containerWidth=container.offsetWidth;const containerHeight=container.offsetHeight;const containerAspectRatio=containerWidth/containerHeight;const videoAspectRatio=videoWidth/videoHeight;if(containerAspectRatio>videoAspectRatio){iframe.style.width="100%";iframe.style.height=`${containerWidth / videoAspectRatio}px`;}else{iframe.style.width=`${containerHeight * videoAspectRatio}px`;iframe.style.height="100%";}
iframe.style.top="50%";iframe.style.left="50%";iframe.style.transform="translate(-50%, -50%)";});});}
player.on("play",()=>{poster.style.display="none";iframe.style.opacity="1";setTimeout(()=>{poster.style.display="none";},500);resizeVideo();});window.addEventListener("resize",resizeVideo);}
