...
const textTimeline=[{time:0,text:"первого погружения с аквалангом в Красном море"},{time:0.99,text:"романтического ужина на пляже Мальдив"},{time:1.88,text:"полета на воздушном шаре над Каппадокией"},{time:2.66,text:"купания со слонами в Таиланде"},{time:3.38,text:"завтрака в бассейне на вилле в Дубае"},{time:4.04,text:"первого урока серфинга на Бали"},{time:4.67,text:"прогулки на яхте по побережью Дубая"},{time:5.29,text:"первого заплыва с черепахами на Мальдивах"},{time:5.91,text:"посещения термальных источников Памуккале"},{time:6.55,text:"дегустации местной кухни в Стамбуле"},{time:7.24,text:"рыбалки на закате в Индийском океане"},{time:7.98,text:"первого урока дайвинга в бассейне"},{time:8.82,text:"спа-ритуала с видом на океан"},{time:9.75,text:"хаммама в лучших традициях Турции"},{time:10.8,text:"тайского массажа на пляже"},];async function loadInto(){let controller=new AbortController();let{signal}=controller;intro:try{let loadedProgressSteps=0;const allProgressSteps=2;function updateProgress(loadedCount,allCount){setLoadProgress((loadedProgressSteps+loadedCount)*100/(allProgressSteps+allCount))}
console.log("🔹 Step 0: get intro show flag");introFlags=await loadStep([checkIntroFlags(),loadCSS("css/bootstrap.element.min.css"),loadScript("https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js")],signal);if(introFlags.includes("returned")){await loadStep([prepareSkipButtons(".slide-container.zero-slide ~ [class*='-slide']",controller),],signal);}
else if(introFlags.includes("skip")){console.log("skipping intro");skipIntro();break intro;}
console.log("🔹 Step 2: load zero slide css");await loadStep([loadCSS("css/zero.min.css"),],signal);document.body.style.display='block';console.log("🔹 Step 3: load lazy load js");await loadStep([loadScript("https://cdn.jsdelivr.net/npm/vanilla-lazyload@19.1.3/dist/lazyload.min.js","LazyLoad"),],signal);console.log("🔹 Step 4: load zero slide resources");await loadStep([loadResources(".slide-container.zero-slide .lazy",null),],signal);loadedProgressSteps++;updateProgress(0,allProgressSteps);console.log("🔹 Step 5: load main css");await loadStep([loadCSS("css/style.min.css"),loadScript("https://cdnjs.cloudflare.com/ajax/libs/fitty/2.3.2/fitty.min.js","fitty")],signal);loadedProgressSteps++;updateProgress(0,allProgressSteps);console.log("🔹 Step 6: load intro slides resources");const step6Results=await loadStep([loadResources(".slide-container.zero-slide ~ [class*='-slide'] .lazy",updateProgress),loadCSS("css/bootstrap.element.min.css"),loadScript("https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"),initSprite()],signal);console.log("🔹 Step 7: waiting resources progress");await loadStep([waitForProgressCompete(),],signal);loadCSS("css/element-icons.min.css");console.log("🔹 Step 8: blue part animation");await loadStep([showBluePartAnimation(),],signal);console.log("🔹 Step 9: hide zero slide and begin slideshow");await loadStep([removeSlide(document.querySelector(".slide-container.zero-slide")),startSlidesCycle(".slide-container.zero-slide ~ [class*='-slide']",step6Results[3])],signal);}
catch(error){if(error.name==="AbortError"){console.log("⏩ Loading skipped by user");skipIntro();}else{console.error("❌ Error loading page:",error);}}
controller=new AbortController();signal=controller.signal;try{console.log("🔹 Step 10: loading main page scripts");await loadStep([loadScript("https://cdn.jsdelivr.net/npm/vanilla-lazyload@19.1.3/dist/lazyload.min.js","LazyLoad"),loadCSS("css/bootstrap.element.min.css"),loadScript("https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"),loadScript("https://player.vimeo.com/api/player.js")],signal);console.log("🔹 Step 11: loading main page resources");const step11Results=await loadStep([loadCSS("css/style.min.css"),loadResources(".video-banner-poster-lazy"),initLazyLoad(),],signal);window.lazyLoadInstance=step11Results[2];console.log("🔹 Step 12: showing page and lazyload remaining resources");const step12Results=await loadStep([enableMainPage(),showMainPage(),loadCSS("css/element-icons.min.css")],signal);}
catch(error){if(error.name==="AbortError"){console.log("⏩ Loading skipped by user");await showMainPage();}else{console.error("❌ Error loading page:",error);await showMainPage();}}}
...
const onAbort=()=>{reject(new DOMException("Aborted","AbortError"));};signal?.addEventListener("abort",onAbort,{once:true});Promise.all(promises.map(p=>p.catch(err=>{console.error("Ошибка в loadStep:",err);throw err;}))).then(resolve).catch(reject).finally(()=>{signal?.removeEventListener("abort",onAbort);});});}
...
slide.addEventListener('animationend',(event)=>{if(event.animationName==='showHideSlide'){console.log(`Завершена анимация скрытия слайда ${slide.classList}`);removeSlide(slide);slidesCount--;console.log("slidesCount = ",slidesCount);if(!slidesCount){resolve();}}});});}
...
function setTransformOriginToLetter(container,letter){const rows=Array.from(container.querySelectorAll('.text-row'));let totalOffsetY=0;for(const row of rows){if(row.textContent.includes('впечатлений')){const letterIndex=row.textContent.indexOf(letter);if(letterIndex===-1){totalOffsetY+=row.getBoundingClientRect().height;continue;}
...
if(Math.abs(currentWidth-lastWidth)>1){container.style.width=`${currentWidth}px`;lastWidth=currentWidth;iterations++;updateTextAndCheckHeight();}else if(containerSelector.includes("fourth-slide")){setTransformOriginToLetter(container,"т");}});}
...
return null;}
function activateTabByHash(hash){const country=decodeURIComponent(hash.replace("#",""));console.log("Активируем таб:",country);const targetTab=[...navLinks].find(link=>{return new URL(link.href).hash.replace("#","")===country;});if(targetTab){targetTab.click();}}
...
return 300;}}
const reviews=[{id:1,name:"Любовь",avatar:"images/review_avatar_1_thumb@3x.avif",text:"Вот уже больше 10 лет путешествуем семьей только по рекомендациям Маши. Я сложной и самый капризный клиент. Могу очень долго выбирать отель и задавать кучу вопросов. Терпению Маши можно позавидовать, всегда найдет вариант, который меня устроит. Мы летали Турцию, Египет, Доминикану, Мексику, Маврикий, Таиланд. Мне нравится что Маша легко ориентируется в большом количестве направлений и с легкостью может рассказать об особенностях страны, сезонности, достопримечательностях.",tripDescription:"Мексика, Маврикий, Египет, Турция",socialPlatform:"instagram",socialLink:"https://www.instagram.com/luba.borovkova"},{id:2,name:"Татьяна",avatar:"images/review_avatar_2_thumb@3x.avif",text:"Каждая поездка начинается с сильных разногласий в семье, у мужа горы, у меня море. У меня 5 звезд, у мужа максимум 4, но спасибо Марии она может деликатно склонить моего мужа в нужном для меня отпускном направлении 😄👌🏻Девочки, надеюсь вы понимаете, что это бесценно? 😂  И да, семья пока держится, опять таки спасибо кому? Спасибо Марии ❣️😚❣️",tripDescription:"Шри Ланка, Мальдивы, Китай, ОАЭ",socialPlatform:"instagram",socialLink:"https://www.instagram.com/design_levina"},{id:3,name:"Анастасия",avatar:"images/review_avatar_3_thumb@3x.avif",text:"Хочу поделиться своими впечатлениями о долголетнем сотрудничестве с Марией. С Машей мы уже отдыхаем больше пятнадцати лет, пишу именно отдыхаем, так как Маша каждый наш отпуск всегда на связи. Все возникающие вопросы и проблемы решаются нашей феей мгновенно. Маша всегда прислушивается к нашим пожеланиям и подбирает идеальные варианты для нас. Всем своим друзьям давно уже рекомендовала Марию, как самого чуткого ,ответственного, честного агента, да и просто, как прекрасную девушку. Спасибо Маша, впереди у нас еще много путешествий!",tripDescription:"Египет, Мальдивы, Таиланд, Бали, Китай, Гонконг, ОАЭ",socialPlatform:"instagram",socialLink:"https://www.instagram.com/nastyshadiamond"},{id:4,name:"Светлана",avatar:"images/review_avatar_4_thumb@3x.avif",text:"Я очень благодарна турагентству «Элемент тревел» за организацию моего отдыха. Уже на протяжении нескольких лет мне и моей семье помогают в подборе тура, всегда оперативно отвечают на запросы, подбирают удобные мне варианты по бюджету и времени. Всегда на связи и своевременно помогают с разными организационными вопросами. Проводят огромную работу по подбору тура, причем делают это в удобное для меня время, далеко нерабочее для большинства людей. Терпеливо отвечают на все вопросы, грамотно развеивают мои сомнения, стараются учесть все пожелания, помогают зарегистрироваться на рейс, способствуют быстрому размещению в отеле. Признательна за неравнодушие и обязательно буду рекомендовать данное турагентство!",tripDescription:"Мальдивы, Египет, Турция, ОАЭ",socialPlatform:"instagram",socialLink:"https://www.instagram.com/svetlanapashchenco"},{id:5,name:"Марина",avatar:"images/review_avatar_5_thumb@3x.avif",text:"Хочу поделиться впечатлениями о своих замечательных поездках, которые нам организовала Мария К. Даже посетив наш любимый отель в Турции, но в первый раз с ее помощью сразу заметили разницу. Был предложен и заказан индивидуальный трансфер, выбраны места в самолете, отель был предупрежден, что мы постоянные гости. Отдых начался еще с самого вылета. В остальных поездках мы уже полностью доверились в руки профессионала и просто наслаждались новыми впечатлениями от Дубая и прекрасным отелем JA PALM TREE COURT. Мария предложила самый комфортный для нас перелет, договорилась с отелем о раннем заселении в отель, учла наши пожелания о наличии спортивных активностей. Сейчас планируем на лето круиз по Средиземному морю. Верю, что это будет незабываемо!",tripDescription:"Турция, Египет, ОАЭ",socialPlatform:"instagram",socialLink:"https://www.instagram.com/semavinamarina_artist"}];function getSocialIcon(platform){return platform==='instagram'?'<svg width="24"height="24"><use href="#ig"/></svg>':'<svg width="24"height="24"><use href="#ext_link"/></svg>';}
